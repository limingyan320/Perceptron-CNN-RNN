...
async def fetch_one(session: aiohttp.ClientSession, index: int, sem: asyncio.Semaphore) -> dict:
    async with sem:
        ...
        for attempt in range(1,RETRY_COUNT + 2):
            try:
                async with session.get(BASE_URL, params=params) as resp:
                    ...
                    filename = f"captcha_{index:04d}{ext}" 
...
async def main() -> None:
    ...
    headers = {
        "User-Agent": "captcha-collector-learning/0.1",
    }
    ...
    async with aiohttp.ClientSession(
        ...
        tasks = [
            fetch_one(session = session,index = i,sem = sem)
